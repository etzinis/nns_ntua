...
%for k = 1:6
    for j = 1:4
        ...
        for i = 1:5
            ...
            if j == 1 
                network = newff(pr_train, train_d_targets, [20, 15], {'tansig', 'tansig', 'tansig'});
            elseif j == 2
                network = newff(pr_train, train_d_targets, [20, 15], {'tansig', 'tansig', 'logsig'});
            elseif j == 3
                network = newff(pr_train, train_d_targets, [20, 15], {'tansig', 'tansig', 'purelin'});
            elseif j == 4
                network = newff(pr_train, train_d_targets, [20, 15], {'tansig', 'tansig', 'hardlim'});    
            end
